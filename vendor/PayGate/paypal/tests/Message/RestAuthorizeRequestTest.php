...
class RestAuthorizeRequestTest extends TestCase
{
    ...
    public function testGetDataWithoutCard()
    {
        ...
        $this->assertSame('9.99', $data['transactions'][0]['amount']['total']);
    ...
    public function testGetDataWithCard()
    {
        ...
        $card->setStartYear(2011);

        $this->request->setCard($card);
        $this->request->setTransactionId('trans123');
        $this->request->setDescription('Item');
        ...
        $this->assertSame('9.99', $data['transactions'][0]['amount']['total']);
        $this->assertSame('USD', $data['transactions'][0]['amount']['currency']);
        $this->assertSame('trans123 : Item', $data['transactions'][0]['description']);
    ...
    {
        $this->request->setAmount('97.00');
        $this->request->setCurrency('USD');
        $this->request->setItems(array(
            array('name' => 'Item1', 'description' => 'descr1', 'quantity' => 1, 'price' => 20),
            array('name' => 'Item2', 'description' => 'descr2', 'quantity' => 2, 'price' => 77),
        ));

        $data = $this->request->getData();
        $transactionData = $data['transactions'][0];

        $this->assertSame('Item1', $transactionData['item_list']['items'][0]['name']);
        $this->assertSame('descr1', $transactionData['item_list']['items'][0]['description']);
        $this->assertSame(1, $transactionData['item_list']['items'][0]['quantity']);
        $this->assertSame('20.00', $transactionData['item_list']['items'][0]['price']);

        $this->assertSame('Item1', $transactionData['item_list']['items'][1]['name']);
        $this->assertSame('descr2', $transactionData['item_list']['items'][1]['description']);
        $this->assertSame(2, $transactionData['item_list']['items'][1]['quantity']);
        $this->assertSame('77.00', $transactionData['item_list']['items'][1]['price']);

        $this->assertSame('97.00', $transactionData['amount']['total']);
    ...
    public function testDescription()
    {
        ...
        $this->request->setTransactionId('');
        $this->request->setDescription('Item');
        $this->assertEquals('Item', $this->request->getDescription());

        $this->request->setTransactionId('trans123');
        $this->request->setDescription('');
        $this->assertEquals('trans123', $this->request->getDescription());

        $this->request->setTransactionId('trans123');
        $this->request->setDescription('Item');
        $this->assertEquals('trans123 : Item', $this->request->getDescription());
    }
}
